...
function split(nid, cb) {
    ...
    `, (err, res) => {
        ...
        for (let seg of segs) {
            ...
            var segLength = 0;
            for (let s of seg) {
                segLength += turf.lineDistance(s.network.geometry);
            };
            ...
            if ( segLength !== 0 ) {
                ...
                try {
                    ...
                    var err = new Error("Unable to interpolize: " + JSON.stringify({
                        "text": text,
                        "seg": seg,
                        debug: opts.debug,
                        post: opts.post,
                ...
                var err = new Error("Unable to interpolize, segment has length of zero: " + JSON.stringify({
                    "text": text,
                    "seg": seg,
                    debug: opts.debug,
                    post: opts.post,
